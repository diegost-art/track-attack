...
// ---- KONFIGURATION -------------------------------------------------
// Eine Spotify Client ID ist in einer Browser-App bewusst oeffentlich: Sie identifiziert
// die App, aber niemals ein Benutzerkonto. Ein Client Secret gehoert NICHT in diese Datei.
...
async function startLogin() {
  // Nur einen echten Platzhalter blockieren. Die konfigurierte Client ID darf hier
  // keinesfalls als Platzhalter behandelt werden – sonst startet der OAuth-Login nie.
  if (!CLIENT_ID || CLIENT_ID === "DEINE_SPOTIFY_CLIENT_ID") {
    toast("Spotify Client ID fehlt. Bitte app.js pruefen.", 5000);
    return;
  }
  if (!window.isSecureContext) {
    toast("Spotify-Login braucht HTTPS (oder localhost).", 6000);
    return;
  }
...
async function exchangeCodeForToken(code) {
  ...
  if (!res.ok) {
    let detail = "";
    try { const payload = await res.json(); detail = payload.error_description || payload.error || ""; } catch {}
    throw new Error(`Token-Austausch fehlgeschlagen${detail ? `: ${detail}` : ""}`);
  }
...
async function drawSpotifyCard(attempts = 0) {
  ...
  // Spotify kann keinen vollstaendigen Katalog ausliefern. Die Suche zieht jedoch
  // pro Karte eine zufaellige Seite aus dem passenden Jahrzehnt/Genre.
  const parts = [`year:${start}-${end}`];
  if (genre) parts.push(`genre:${genre}`);
  const query = parts.join(" ");
  // Bei wenigen Treffern ist ein grosser Offset ungueltig. Mit max. 50 bleiben
  // Jahrzehnte und Nischen-Genres robust, der Zufall bleibt trotzdem erhalten.
  const offset = Math.floor(Math.random() * 51);

  const res = await spotifyFetch(
    `/search?q=${encodeURIComponent(query)}&type=track&limit=50&offset=${offset}`
  );
  if (res.status === 401 || res.status === 403) {
    toast("Spotify-Zugriff verweigert. Bitte neu verbinden und Premium-/Testerfreigabe pruefen.", 6000);
    return null;
  }
