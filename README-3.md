# Track Attack
## Spotify-Fix in dieser Version

Die Spotify-Client-ID ist jetzt korrekt aktiv. In der vorherigen Version wurde exakt diese ID irrtuemlich wie ein Platzhalter behandelt; deshalb begann die Spotify-Anmeldung nicht und der Live-Modus konnte keine Songs laden.

Vor dem Testen im Spotify Developer Dashboard pruefen:

1. Unter **Settings → Redirect URIs** muss exakt die URL deiner GitHub-Pages-App stehen, zum Beispiel `https://BENUTZERNAME.github.io/REPOSITORY/`. Die URL ohne und mit abschliessendem Slash sind verschiedene Werte.
2. Solange die Spotify-App im Development Mode ist, muss der Spotify-Account des Gastgebers unter **User Management** als Tester eingetragen sein.
3. Die Web-Wiedergabe erfordert Spotify Premium.
4. Nach dem Upload einmal im Privatfenster testen oder die gespeicherten Website-Daten loeschen. `sw.js` hat eine neue Cache-Version und aktualisiert sich normalerweise automatisch.

Die Client ID darf in `app.js` stehen. Sie ist bei einer Browser-App kein Geheimnis. Ein Spotify Client Secret darf dagegen nie in GitHub oder in JavaScript eingefuegt werden.
